...
m3ApiRawFunction(host_wextension) {
    ...
    if ((strcmp(name, WDISPATCH_EXTENSION) == 0 && version == WDISPATCH_VERSION) ||
        ...
        if (ctx->dispatch_ptr == 0) {
            ctx->dispatch_ptr = host_alloc(ctx, sizeof(wdispatch_t), 4);
            wdispatch_t *d = (wdispatch_t*)(ctx->mem + ctx->dispatch_ptr);
            d->version = 1;
            ...
            d->full_h = 240;
            d->stride = 320;
            d->data_ptr = ctx->default_fb_ptr;
        }
        m3ApiReturn(ctx->dispatch_ptr);
    }
